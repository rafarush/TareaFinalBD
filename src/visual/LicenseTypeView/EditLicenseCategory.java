...
public class EditLicenseCategory extends javax.swing.JDialog {
    public EditLicenseCategory(MainScreen parent, LicenseCategory category) {
        super(parent, true);
        initComponents(parent,category);
    }
    private void initComponents(MainScreen parent,LicenseCategory category) {
        ...
        TitleLabel.setText("Editar Tipo-Categoria");
        ...
        AddjButton1.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                int respuesta= JOptionPane.showConfirmDialog(null, "Está seguro que desea editar esta categoria de licencia?", "Confirmar", JOptionPane.YES_NO_OPTION,JOptionPane.QUESTION_MESSAGE);
                if(respuesta==0) {
                    String type = licenseType.getText().equalsIgnoreCase("") ? category.getLicenseType() : licenseType.getText();
                    String category = licenseCategory.getText();
                    if (!type.isEmpty() && !category.isEmpty()) {
                        try {
                            ServicesLocator.getInstance().getLicenseCategoryServices().updateLicenseCategory(new LicenseCategory(type, category));
                            parent.Actualizar(7);
                            dispose();
                        } catch (Exception e) {
        ...
        licenseType.setText(category.getLicenseType());
        ...
        licenseCategory.setText(category.getLicenseCategory());
